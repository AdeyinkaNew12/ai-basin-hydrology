...
fig, axes = plt.subplots(3, 1, figsize=(6.5, 9.5), dpi=600)
...
for ax, handles in zip(
    axes,
    [
        legend_handles(blue_map),
        legend_handles(orng_map),
        legend_handles(grn_map)
    ]
):
    ax.legend(
        handles=handles,
...
# Compact legends below each panel
for ax, handles in zip(
    axes,
    [
        legend_handles(blue_map),
