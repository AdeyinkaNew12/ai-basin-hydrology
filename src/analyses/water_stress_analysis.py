...
- default output folder: /mnt/disk3/aoolaseinde/projects/final_results/water_stress_analysis
...
DEFAULT_OUTPUT_ROOT = "/mnt/disk3/aoolaseinde/projects/ai-basin-hydrology/final_results/water_stress_analysis"
...
OUT_DIR = Path("/mnt/disk3/aoolaseinde/projects/ai-basin-hydrology/final_results/water_stress_analysis")
...
def plot_grouped_percent_only(
    ...
    for i, cls in enumerate(classes):
        ...
        if SHOW_PERCENT_LABELS:
            ...
            # ------------------------------------------------
            # Common CONUS reference percentages
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(classes, fontsize=9.0)
    ax.tick_params(axis="y", labelsize=9.0)
    ax.set_title(title, fontsize=10.0, fontweight="bold", pad=3)
    clean_axis(ax)
...
def run_mode(mode_name):
    ...
    for i, sec in enumerate(SECTORS):
        ...
        chi2_3, p_3, _, _ = chi2_contingency(
            np.vstack([obs3.values, rnd3.values])
        )

        a = int(obs3["high"])
        b = int(obs3["low"] + obs3["medium"])
        c = int(rnd3["high"])
