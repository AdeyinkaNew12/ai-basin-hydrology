...
def load_points_csv_robust(path):
    ...
    if lat_col is None or lon_col is None:
        raise ValueError(f"Could not detect lat/lon in {path}")
...
def load_power_excel_fixed(path):
    ...
    sheet = next((s for s in xls.sheet_names if "plant" in s.lower()), xls.sheet_names[0])
    ...
    if lat_col is None or lon_col is None:
        raise ValueError(f"Could not detect lat/lon in Power sheet '{sheet}'")
...
def assign_basins(points_gdf, basins_gdf, basin_id_field):
    j = gpd.sjoin(
        points_gdf,
        basins_gdf[[basin_id_field, "geometry"]],
        how="left",
        predicate="within"
    )
...
if id_field is None:
    raise ValueError("Could not find HYBAS_ID in basins.")
...
bm = (
    bas_master
    .merge(ai_b, on=id_field, how="left")
...
if (not RECOMPUTE_NWM) and os.path.exists(HYDRO_OUT):
    print("✅ Using cached hydrologic metrics:", HYDRO_OUT)
    print("Set RECOMPUTE_NWM = False to regenerate from raw NWM Zarr.")
    bas_metrics = pd.read_csv(HYDRO_OUT)

else:
    print("Regenerating hydrologic metrics from NWM. This may take hours.")
    ...
    try:
        ds = xr.open_zarr(
            mapper,
            consolidated=True,
            chunks={"time": TIME_CHUNK, "feature_id": FEAT_CHUNK}
        )
    except Exception:
        ds = xr.open_zarr(
            mapper,
    ...
    feat_metrics_df = feat_metrics_df.merge(
        top_reach_small,
        on="feature_id",
        how="left"
...
drop_cols = [
    "n_AI", "AI_present",
    "n_Power", "Power_present",
    "n_TRI", "TRI_present",
    "area_km2"
]

bas_metrics_clean = bas_metrics.drop(
    columns=[c for c in drop_cols if c in bas_metrics.columns],
    errors="ignore"
)
...
for i, m in enumerate(plot_metrics):
    ...
    if xa.size:
        ax.plot(xa, ya, lw=3.0, color=COLORS["AI"], label=f"AI (N={len(g_ai):,})")
    if xp.size:
        ax.plot(xp, yp, lw=3.0, color=COLORS["Power"], label=f"Power (N={len(g_pw):,})")
    if xt.size:
        ax.plot(xt, yt, lw=3.0, color=COLORS["TRI"], label=f"TRI (N={len(g_tri):,})")
    if xn.size:
        ax.plot(xn, yn, lw=2.4, color=COLORS["None"], ls="--", alpha=0.85, label=f"None (N={len(g_none):,})")
